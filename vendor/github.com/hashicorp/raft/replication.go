...
// follewerResplication负责在leader任期内将快照和log entries发送给远程follower
type followerReplication struct {
	...
	// requests.
	// 当前leader的任期，被包含在AppendEntries请求中
	currentTerm uint64
	...
	// commitment 用于追踪来自followers的日志响应，以便leader提交索引可以进行，
	// 它在成功收到AppendEntries时响应
	...
	// 在每次新的log entries被追加到日志时，triggerCh被通知
	...
	// 无论什么时候收到follower的响应(不管成功或失败)，都需要将lastContact 更新为当前时间。
	// lastContact被用来检查leader是否要卸任
	...
	// failures 用于计算自上次成功依赖失败的 RPCs的次数，它用来apply的回退
	failures uint64
...
// follower.
// replicate 是一个长时间运行的routine，它复制log entries到某个follower
...
// heartbeet 在一端(peer)周期性的调用AppendEntries，以保证他们没有超时
func (r *Raft) heartbeat(s *followerReplication, stopCh chan struct{}) {
	var failures uint64
	// 创建一个AppendEntries请求，日志数据为空
